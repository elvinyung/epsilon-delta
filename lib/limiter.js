...
var epsilonDelta = function (configs) {
  ...
  var rate = function rate (userKey, cb) {
    updateUser(userKey, function (err, data) {
      ...
      return cb(err, limitReached);
    });
  };

  // compiles the datastore key of the user.
  var compileKey = function compileKey(templ, req) {
    return templ.match(/:[\w\.]+/g).reduce(function (acc, key) {
      return acc.replace(
        new RegExp(key, 'g'), 
        utils.nestedGet(req, key.slice(1))
      );
    }, templ);
  }
